...
if __name__ == '__main__':
	...
	for ibatch in xrange(nSimulations):
		...
		nthfile_choice = str(batchVars['nthfile_choice'][ibatch])
		nthfile_list = batchVars['nthfile_list'][ibatch]
		nthfile_seq = batchVars['nthfile_seq'][ibatch]
		...
		offspringfitsurfaceAA = datadir+batchVars['Fitness_AA'][ibatch]
		offspringfitsurfaceAa = datadir+batchVars['Fitness_Aa'][ibatch]
		offspringfitsurfaceaa = datadir+batchVars['Fitness_aa'][ibatch]
		offspringfitsurfaceAABB = datadir+batchVars['Fitness_AABB'][ibatch]
		offspringfitsurfaceAaBB = datadir+batchVars['Fitness_AaBB'][ibatch]
		offspringfitsurfaceaaBB = datadir+batchVars['Fitness_aaBB'][ibatch]
		offspringfitsurfaceAABb = datadir+batchVars['Fitness_AABb'][ibatch]
		offspringfitsurfaceAaBb = datadir+batchVars['Fitness_AaBb'][ibatch]
		...
		if philopatry == 'F' or philopatry == 'female' or philopatry == 'f' or philopatry == '0' or philopatry == 'Female': 
			if sexans != 'Y':
				print('Female philopatry is turned on and sexual reproduction must be specified.')
				sys.exit(-1)
			if equalsexratio != 'AtBirth':
				print('Female philopatry is turned on and equal sex ratio at birth must be specified.')
				sys.exit(-1)
			if len(cdclimgentimelist) > 1:
				if len(np.where(np.asarray(Fdispmovethresh) == '0')[0]) != len(cdclimgentimelist):
					print('Female philopatry is turned on and female dispersal must be set to 0.')
					sys.exit(-1)
			else:
				if Fdispmovethresh != '0':
					print('Female philopatry is turned on and female dispersal must be set to 0.')
					sys.exit(-1)
			if freplace != 'N':
				print('Female philopatry is turned on and females can only mate once.')
				sys.exit(-1)
			if len(cdclimgentimelist) > 1:
				if (len(np.where(np.asarray(Mdispmoveno) == '1')[0]) + len(np.where(np.asarray(Mdispmoveno) == '2')[0]) + len(np.where(np.asarray(Mdispmoveno) == '5')[0]) + len(np.where(np.asarray(Mdispmoveno) == '6')[0])) != len(cdclimgentimelist):
					print('Female philopatry is turned on and male movement must be 1, 2, 5, or 6.')
					sys.exit(-1)
			else:			
				if (Mdispmoveno == '1' or Mdispmoveno == '2' or Mdispmoveno == '5' or Mdispmoveno == '6') == False:
					print('Female philopatry is turned on and male movement must be 1, 2, 5, or 6.')
					sys.exit(-1)
		# For male philopatry
		elif philopatry == 'M' or philopatry == 'male' or philopatry == 'm' or philopatry == '1' or philopatry == 'Male': 
			if sexans != 'Y':
				print('Male philopatry is turned on and sexual reproduction must be specified.')
				sys.exit(-1)
			if equalsexratio != 'AtBirth':
				print('Male philopatry is turned on and equal sex ratio at birth must be specified.')
				sys.exit(-1)
			if len(cdclimgentimelist) > 1:
				if len(np.where(np.asarray(Mdispmovethresh) == '0')[0]) != len(cdclimgentimelist):
					print('Male philopatry is turned on and male dispersal must be set to 0.')
				sys.exit(-1)
			else:
				if Mdispmovethresh != '0':
					print('Male philopatry is turned on and male dispersal must be set to 0.')
					sys.exit(-1)
			if len(cdclimgentimelist) > 1:
				if (len(np.where(np.asarray(Fdispmoveno) == '1')[0]) + len(np.where(np.asarray(Fdispmoveno) == '2')[0]) + len(np.where(np.asarray(Fdispmoveno) == '5')[0]) + len(np.where(np.asarray(Fdispmoveno) == '6')[0])) != len(cdclimgentimelist):
					print('Male philopatry is turned on and female movement must be 1, 2, 5, or 6.')
					sys.exit(-1)
			else:			
				if (Fdispmoveno == '1' or Fdispmoveno == '2' or Fdispmoveno == '5' or Fdispmoveno == '6') == False:
					print('Male philopatry is turned on and female movement must be 1, 2, 5, or 6.')
					sys.exit(-1)
		...
		for ithmcrun in xrange(mcruns):	
			...
			# Prepare fitness surface grid file
			if cdevolveans == '2':
				offspringfitsurfaceAABB = PrepTextFile(offspringfitsurfaceAABB)
				offspringfitsurfaceAaBB = PrepTextFile(offspringfitsurfaceAaBB)
				offspringfitsurfaceaaBB = PrepTextFile(offspringfitsurfaceaaBB)
				offspringfitsurfaceAABb = PrepTextFile(offspringfitsurfaceAABb)
				offspringfitsurfaceAaBb = PrepTextFile(offspringfitsurfaceAaBb)
			...
			xyfilename,agefilename,equalsexratio,loci,intgenesans,allefreqfilename,alleles,0,logfHndl,cdevolveans,offspringfitsurfaceAA,\
			offspringfitsurfaceAa,offspringfitsurfaceaa,cdinfect,Infected,\
			offspringfitsurfaceAABB,offspringfitsurfaceAaBB,\
			offspringfitsurfaceaaBB,offspringfitsurfaceAABb,\
			offspringfitsurfaceAaBb,offspringfitsurfaceaaBb,\
			offspringfitsurfaceAAbb,offspringfitsurfaceAabb,\
			offspringfitsurfaceaabb,subpopmigration,subpopemigration,datadir,geneswap)
			...
			fitvals1 = tupPreProcess[10]
			infection = tupPreProcess[11]
			Infected = tupPreProcess[12]
			fitvals2 = tupPreProcess[13]
			subpopmigration = tupPreProcess[14]
			subpopemigration = tupPreProcess[15]
			agemortvals =  tupPreProcess[16]
			egg_lmbdavals = tupPreProcess[17]
			egg_sigmavals = tupPreProcess[18]
			allelst = tupPreProcess[19]
			newmortperc = tupPreProcess[20]
			...
			for gen in xrange(looptime):
				...
				for icdtime in xrange(len(cdclimgentime)):
					if gen == int(cdclimgentime[icdtime]):
						...
						mateno = matemoveno[icdtime]
						Fdispno = Fdispmoveno[icdtime]
						Mdispno = Mdispmoveno[icdtime] 
				...
				if gen != 0:
					...
					infectionnew,allelst,geneswap,gen)	
				...
				FSDMate,MSDMate,filledgrids,Female_BreedEvents,gen,subpop,BreedFemales_age,agemort,subpopmatemort,subpopmortperc)
				Bearpairs = tupMate[0]
				females = tupMate[1]
				...
				infection,geneswap,popmodel,K_env)
				...
				males,males_nomate[gen],burningen,thresh_F,thresh_M)
			...
			BreedFemales_age,subpopmatemort)
