...
genesnew,equalsexratio,sexnew,subpopnew,infectionnew,allelst,geneswap,gen):	
	...
	# If geneswap is < 0, do nothing, else intialize genes here
	if gen == geneswap:		
...
# ---------------------------------------------------------------------------------------------------
def ConstantMortality(filledgrids,nogrids,sex,id,age,xgrid,ygrid,gen,genes,Deaths,alleles,FID,agemort,infection,geneswap):
	...
	for i in xrange(1,(len(agedeaths)+1)):
		# NA switch
		if len(openindex) == 0:
			deleteoldindex.append(random.sample(np.where(np.asarray(age) == i)[0],int(agedeaths[i-1])))
		else:
	...
	if len(deleteoldindex) != 0:
		...
		FID = np.asarray(FID)
		infection = np.asarray(infection)
...
# ----------------------------------------------------------------------------------------------	 
def DDMortality(filledgrids,nogrids,sex,id,age,xgrid,ygrid,gen,genes,Deaths,alleles,FID,infection,geneswap,K_env,popmodel,agemort):
	'''
...
# ---------------------------------------------------------------------------------------------------	 
def DoMortality(filledgrids,nogrids,sex,id,age,xgrid,ygrid,gen,genes,Deaths,alleles,FID,agemort,infection,geneswap,popmodel,K_env):
	'''
	DoMortality()
	...
	if popmodel == 'exp':
		...
		tupMort = ConstantMortality(filledgrids,nogrids,sex,id,age,xgrid,ygrid,gen,genes,Deaths,alleles,FID,agemort,infection,geneswap)
		
	elif popmodel == 'logistic' or popmodel == 'rickers' or popmodel == 'richards':
		
		tupMort = DDMortality(filledgrids,nogrids,sex,id,age,xgrid,ygrid,gen,genes,Deaths,alleles,FID,infection,geneswap,K_env,popmodel,agemort)		
